...
const UploadFileCard: React.FC<UploadFileCardProps> = ({className}) => {
  ...
  const allowedFormats = fileType === 'Test Case' ? '.csv,.xlsx' : '.docx';

  const fileLimitText =
    fileType === 'Test Case'
      ? 'Limit 200MB .CSV or .XLSX'
      : fileType === 'Process Document'
      ? 'Limit 200MB .DOCX'
      : '';
            ...
            <Form>
              ...
              <Form.Group controlId="fileUpload" className="text-center mt-4">
                ...
                <Form.Control
                  ...
                  className="d-none"
                />
                <Button variant="outline-primary" className="upload-btn" onClick={() => document.getElementById('fileUpload')?.click()}>
                  Upload File
                </Button>
                {file && (
                  <div className="mt-3">
